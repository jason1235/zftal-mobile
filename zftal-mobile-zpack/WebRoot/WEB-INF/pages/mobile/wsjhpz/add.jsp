...
<html>
  <head>
   ...
   $(function(){
   		//openAlert("11111", "s", "auto");
    	//$("#jgzx1").hide();
		//$("#jgzx2").hide();
		$("#divul ul li a").click(function(){
			initErrorAlert();
			if($(this).attr("id")==2 && $("#params_wsjhmc").val()==-1){
				errorAlert("配置还未保存，不能进行后续配置！");
				return false;
			}
			
			if($(this).attr("id")!=2 && $("#wsjhmc").val()==-1){
				errorAlert("配置还未保存，不能进行后续配置！");
				return false;
			}
			$("#divul ul li").removeClass("active");
			$(this).closest("li").addClass("active");
			$(".tab-pane").removeClass("active");
			var idval = $(this).attr("id");
			if(idval == '1'){
				$("#tab_lis1").addClass("active");
			}else if(idval == '2'){
				$("#tab_lis2").addClass("active");
			}else if(idval == '3'){
				$("#tab_lis3").addClass("active");
			}else{
				$("#tab_lis4").addClass("active");
			}
		});
		
		$("#2").click(function(){			
			var param_id = $('select[id="params_id"]').val();				
			if($("#params_wsjhmc").val()==-1||param_id==""){
				return false;
			}	
			if($('#paramsflag').val()==1){
				return false;
	        ...
	        var params = "mdbs="+mdbs+"&mdb="+mdb+"&params="+$('select[id="params_id"]').val();
 	      
	        $.ajax({
			       url:'<%=request.getContextPath() %>/webservice/wsjhconfig_getTableFieldsAndParams.html',
			       type:'POST',         //数据发送方式
			       ...
			       success:function(data){
					...
					if(n.split(":")[0] != "time" && n.split(":")[0] != "verify"){
							var retu_html ="";
							var value=n.split(":")[1];
							if(value==1){
								retu_html="<input type='text' name='PAGE' id='PAGE' value='1'/>";
							}else if(value==2){
								retu_html="<input type='text' name='PAGESIZE' id='PAGESIZE' value='10'/>";
							}else if(value==3){
								var sign =$('#wsjhConfig_sign').val();
								retu_html="<input type='text' name='SIGN' id='SIGN' value='"+sign+"'/>";
							}else{
								retu_html = "<select  class='params_mdbz'>"+retu+"</select>";
							}
		...
		$("#params_save").click(function(){			
			...
			$("#paramsTbody").children("tr").each(function(i){			
				var fields = $.trim($(this).find(".params_fields").attr("title"));	
				 types+= $('select[class="params_types"]').val()+",";
				if('PAGE'==fields){
					csz+=$('#PAGE').val()+",";
			...
			$.post("<%=request.getContextPath() %>/webservice/wsjhconfig_saveWsjhParamsConfig.html",
					{"wsjhmc":encodeURI($("#wsjhConfig_wsjhmc").val()),"pfields":pfields,"csz":csz,"types":types},
				function(data){
		   ...
		   // $.ajax({
		      // url:'<%=request.getContextPath() %>/webservice/wsjhconfig_saveWsjhParamsConfig.html',
		       //type:'POST',         //数据发送方式
		       //dataType:'text',     //接受数据格式
		...
		$("#3").click(function(){
			...
			var params = "mdbs="+mdbs+"&mdb="+mdb+"&variable="+variable+"&params="+$('select[id="params_id"]').val();
			$.ajax({
		       url:'<%=request.getContextPath() %>/webservice/wsjhconfig_getTableFieldsAndTypes.html',
		       type:'POST',         //数据发送方式
		       dataType:'text',     //接受数据格式
		       data:params,
		       cache:false,
		       success:function(data){
		       		...
		       		$.each(array[2].split("@"),function(i,n){		       				       			
		       			var html = "<select  class='mbzdsjlx'>"+$("#option").html()+"</select>";
		       			var retu_html = "<select  class='retu'>"+retu+"</select>";
		       			table_tbody.append("<tr onclick=\"setRowId('dztr"+i+"')\" id='dztr"+i +"' style='cursor:hand'>"		       				   
		       			+"<td class='mbzd' title="+n.split(":")[0]+">"+n.split(":")[0]+"</td>"
		       			+"<td>"+html+"</td>"	
		       			+"<td class='variable' >"+retu_html+"</td>"
		       			+"<td><input type='checkbox' class='sfzj'></td>"
		       			...
		       			$("#dztr"+i).find(".mbzdsjlx").val(n.split(":")[2]);		       			 
		       		});		   
		...
		//字段对照 保存按钮
		$("body").on("click","#zddzSave", function () { 
		//$("#zddzSave").click(function(){
			...
			$("#zddzTbody").find("tr").each(function(i){
			   var lens=$(this).val();
			   if(lens!=""){
			...
			$.post("<%=request.getContextPath() %>/webservice/wsjhconfig_saveWsjhdzConfig.html",
					{"wsjhmc":encodeURI($("#wsjhConfig_wsjhmc").val()),"variables":variable,"mbzd":mbzd,"mbzdsjlx":mbzdsjlx,"sfzj":sfzj,"sfgx":sfgx
	    			 ,"dsqzt":dsqzt,"jhId":jhId},function(data){
	    			var callback = function(){
	    				$('#zddzflag').val(1);
						$("#a[id^=start]").show();
						$("#a[id^=stop]").hide();
		...
		//隐藏实时交换
		if($(".sfssjh").prop("checked")==0){
			$("#ssjh").hide();
			
		}
		//是否实时交换改变事件
		$(".sfssjh").click(function(){
			if($(".sfssjh").prop("checked")==1){
				$("#ssjh").show();
			}else{
				$("#ssjhlx").val("");
				$("#bzwzd").val("");
		...
		$('#wsjh_load_id').click(function(){
		    ...
		    if(url==null||url.length==0){
		    	...
		    	$.post("webservice/wsjhconfig_load.html",{"url":url},function(data){	    		
		    		...
		    		var wsjhConfig_operate_html = "<select name='wsjhConfig.operate' id='wsjhConfig_operate'  class='form-control'>";
		    		var wsjhConfig_params_html="<select name='wsjhConfig.params' disabled id='params_id'  class='form-control'>";
		    		var wsjhConfig_types_html="<select name='wsjhConfig.types' disabled id='params_type'  class='form-control'>";
	    ...
	    $('#wsjh_save_id').click(function(){
	    	...
	    	var operate = $("select[name='wsjhConfig.operate']").val();
	    	if(operate==null||operate.length==0){
	    		errorAlert("请输入WebService  操作方法!");
	    		return false;
	    	}
	    	var wsjhb = $('#mdb').val();
	    	if(wsjhb==null||wsjhb.length==0){
	    		errorAlert("请输入目的表!");
	    		return false;
	    	}		
			validate();
	    	...
	    	var isSjc = $('input[name="wsjhConfig.isSjc"]:checked').val();    
	    	var isType=$('input[name="wsjhConfig.isType"]:checked').val();
	    	var gys = $('#wsjhConfig_gys').val();
	    	var zys = $('#wsjhConfig_zys').val();
	    	var oflag = $('input[name="wsjhConfig.oflag"]:checked').val();
	    	$.post("webservice/wsjhconfig_save.html",{"wsjhmc":encodeURI(wsjhmc),"mdb":mdb,"wsjhb":$('#mdb').val(),"wsjhzcb":$('#pmdb').val(),"sjcz":sjcz,"isSjc":isSjc
	    		,"url":$('#wsjhConfig_url').val(),"namespace":$('#wsjhConfig_namespace').val(),"operate":operate,"params":params,"fhbl":fhbl,"isType":isType,
	    		 "gys":gys,"zys":zys,"oflag":oflag},function(data){
	    			var callback = function(){
	    				if(params==null||params.length==0){
		    				$('#wsjhmc').val(1);
		    			}
		    			$('#params_wsjhmc').val(wsjhmc);
		    			$('#2').trigger("click");
					};
	    ...
	    $("#wsjh_fhbl_id").click(function(){
	    	var isType=$('input[name="wsjhConfig.isType"]:checked').val();
	    	var types = $('#params_type').val();
	    	var gys="";
	    	var zys="";
	    	if(isType==1){//1xml 0json 2object
	    		 gys = $('#wsjhConfig_gys').val();
	    	     zys = $('#wsjhConfig_zys').val();
	    	     //if(gys==null||gys.length==0){
	    	     //	 errorAlert("返回类型为XML，根元素不能为空");
	    	     //	 return  false;
	    	     //}
	    	     /*if(zys==null||zys.length==0){
	    	    	 errorAlert("返回类型为XML，子元素不能为空");
	    	    	 return false;
	    	     }*/
	    	}
	    	...
	    	if(params==null||params.length==0){
		    	...
		    	var operate = $("select[name='wsjhConfig.operate']").val();  
		    	$.post("<%=request.getContextPath() %>/webservice/wsjhconfig_getFhbl.html",{"url":url,"namespace":namespace,"operate":operate,"isType":isType,"gys":gys,"zys":zys,"types":types},function(data){
					var params = data.params;
					$('#wsjhConfig_fhbl').val(params);
	        ...
	        $("#myFrame").attr("src", "<%=request.getContextPath() %>/webservice/wsjhconfig_getParamsPage.html?params="+params+"&namespace="+namespace+"&operate="+operate+"&url="+url+"&types="+types);
		...
		$("body").on("change","select[id='wsjhConfig_operate']", function () { 
			...
			var types = document.getElementById("params_type");		
			$('#params_type').val(types.options[index].value);
		});
	    $("#mdbView").click(function(){
	        ...
	        $("#myModal_toji").find(".modal-content").height(700);
	...
	function validate(){
		...
		//$('#wsjhConfig_wsjhmc').live('blur',function(){
			var wsjhmc = $(this).val();
			if(wsjhmc==null||wsjhmc.length==0){
				errorAlert("请输入WebService 交换配置名称!");
				...
				$.post("webservice/wsjhconfig_varifyWsjhmc.html",{"name":encodeURI(wsjhmc)},function(data){				
					if(data.result=='true'){
						errorAlert("该WebService 交换配置名称已存在！请重新输入");
						$('#wsjhConfig_wsjhmc').focus();
						$('#wsjhConfig_wsjhmc').val('');
						return false;
					}else{
						return true;
					}
		...
		//$('#wsjhConfig_namespace').live('blur',function(){
	...
	function startRunset(dsqxh,status){
		var lt = $("#zddzTable tbody tr").length;
		if(status==1&&lt==0){
			errorAlert("字段对照为空，定时器无法启动！");
			return false;
		}
		$.ajax({
  ...
  <div class="modal-header">
                <button type="button" class="close"><span aria-hidden="true" class="glyphicon glyphicon-share-alt">&times;</span><span class="sr-only">Close</span></button>
                <h4 class="modal-title text-left" id="myModalLabel">webservice增加</h4>
            </div>
			...
			<tbody>
              <tr>
                 <th width="30%" >
                  ...
                  <td colspan="3">
                     <input id="wsjhConfig_wsjhmc" maxlength="30" placeholder="不要超过30个字符哦(^_^)" name="wsjhConfig.wsjhmc" type="text" class="form-control"/>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td colspan="3">
                  	<span>
                     <input id="wsjhConfig_url" maxlength="100" placeholder="不要超过100个字符哦(^_^)" name="wsjhConfig.url" type="text" style="width: 80%;"/>
              ...
              <tr>
                 <th width="30%" >
                  ...
                  <td colspan="3">
                     <input id="wsjhConfig_namespace" maxlength="60" placeholder="不要超过60个字符哦(^_^)" name="wsjhConfig.namespace" type="text" class="form-control"/>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td id="wsjhConfig_operate_td" colspan="3">
                   <select id="wsjhConfig_operate" name="wsjhConfig.operate"  class="form-control">
                      <option value=""></option>
                   </select>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td id="wsjhConfig_params_td" colspan="3">
                   <select id="params_id" disabled name="wsjhConfig.params"  class="form-control">
                      <option value=""></option>
                   </select>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                     <span class="red">*</span>方法参数类型:
                 </th>
                  <td id="wsjhConfig_types_td" colspan="3">
                   <select id="params_type" disabled name="wsjhConfig.types"  class="form-control">
                      <option value=""></option>
                   </select>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td colspan="3">
                   <span>
	                      	<input type="radio" name="wsjhConfig.isType" value="1" id="wsjhConfig_isType_1" >
	                      	XML
	                      	</input> &nbsp;&nbsp;&nbsp;&nbsp;
	                      	<input type="radio" id="wsjhConfig_isType_2" name="wsjhConfig.isType"  value="0" checked>
	                      	JSON
	                      	</input>
                   </span>
                   ...
                   <span>
                   		...
                   		根元素：<input id="wsjhConfig_gys" maxlength="15" placeholder="不要超过15个字符哦(^_^)" name="wsjhConfig.gys" type="text" style="width: 30%;"/>
                   		子元素：<input id="wsjhConfig_zys" maxlength="15" placeholder="不要超过15个字符哦(^_^)" name="wsjhConfig.zys" type="text" style="width: 30%;"/>
                   </span>
                   </div>
                 </td>
              </tr>	
              <tr>
                 <th width="30%" >
                  ...
                  <td colspan="3">
                     <textarea rows="1" cols="20" name="wsjhConfig.fhbl" id="wsjhConfig_fhbl" readonly="true" style="width: 80%;height: 50px;"></textarea>
              ...
              <tr>
                 <th width="20%" >
                  ...
                  <td  width="30%">
                  <input type="radio" name="wsjhConfig.isSjc" value="1" id="wsjhConfig_isSjc_1">
                  		是
             	  </input> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	
             	  <input type="radio" id="wsjhConfig_isSjc_2" name="wsjhConfig.isSjc"  value="0" checked>
             	  		否
             	  </input>
			         <div id="wsjhConfig_jsSjc_div" style="display:none;float:right">
			         	<input type="text" id="wsjhConfig_sjcz" name="wsjhConfig.sjcz"  style="width:100px;"/>
			         </div>
			     </td>
	                 ...
	                 <td width="30%"> 
	                 <select id="mds_select_id" name="wsjhConfig.wsjhds" class="form-control">
			     ...
			     <tr>
	                 ...
	                 <td width="30%">
	                 <span>
	                 	<input type="text" id="pmdb" name="wsjhConfig.wsjhzcb" readonly="true" style="width: 80%;"/>
	     				<a href="#" id="pmdbView">
	     				✚
	     				</a>
	                 ...
	                 <td width="30%">
	                 	<input type="text" id="mdb" name="wsjhConfig.wsjhb" readonly="true" style="width: 80%;"/>
	     				<a href="#" id="mdbView">
	     				✚
	     				</a>
                 </tr>
                 ...
                 <th width="30%" >
                  ...
                  <td colspan="3">
                  <input type="radio"  name="wsjhConfig.oflag" value="1">
                 		 是
                  </input> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <input type="radio"  name="wsjhConfig.oflag" checked value="0">
                 		 否
                  </input>
			     </td>
			     </tr>
  ...
  <div role="tabpanel" class="tab-pane" id="tab_lis3">
  		<input type="hidden" id="wsjhmc" name="wsjhConfig.wsjhmc" value="-1"/>
