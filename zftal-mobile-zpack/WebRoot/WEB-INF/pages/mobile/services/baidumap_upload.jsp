...
<form id="uploadForm" class="am-form am-margin" onsubmit="return doSubmit();" action="<%=request.getContextPath() %>/serviceManager/gaoDeMap_imp.html" target="resultFrame" method="post" enctype="multipart/form-data">
<div class="am-form-group am-form-file">
	<a alt="模板下载" href="<%=request.getContextPath() %>/upload/baidumap.xls">地图数据表格模板下载，请点击此处下载</a>
...
function doSubmit() {
	...
	if(filename == null || filename == "" || filename==undefined){
		...
		alert("上传文件格式不对，您导入的不是excel文件，请导入excel文件！");
		return false;
	}
}
  ...
  $(function() {
	  $("#submit").click(function(){
	      ...
	      $.ajax({
	    	    url: '<%=request.getContextPath() %>/serviceManager/gaoDeMap_imp.html',
	    	    type: 'POST',
	    	    cache: false,
...
<jsp:include page="/WEB-INF/pages/globalweb/modal.jsp" flush="true"></jsp:include>
	...
	<form action="<%=request.getContextPath() %>/serviceManager/gaoDeMap_upload.html" name="search" id="search" method="post">
		 <div class="form-horizontal sl_all_form">
            <div class="row">
                <div class="col-xs-4">
                    <div class="form-group">
                        <label for="" class="col-xs-5 control-label">校区名称</label>
                        <div class="col-xs-7">
                            <input id="xtbm" maxlength="100" placeholder="最多100个字符" class="form-control" name="query.name" value="${query.name }"/>
                        </div>
                    </div>
                </div>
          ...
          --%><table class="table table-striped sl_all_bg table-bordered text-left" >
            ...
            <tbody  id="list_body">
                ...
                <tr name="tr" class="data">
                    ...
                    <td class="autocut" >${menu.longitude}</td>
                    <td class="autocut" >${menu.latitude}</td>
                </tr>
                </s:iterator>
            </tbody>
        </table>
