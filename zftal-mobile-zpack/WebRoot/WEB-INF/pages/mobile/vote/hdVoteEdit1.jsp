...
<style>
    ...
    $(function(){
        $("#save_btn").click(function(){
            ...
            $.post("<%=request.getContextPath()%>/vote/vote_hdVoteUpdate.html",$("#form_edit").serialize(),function(data){
        ...
        <div class="sl_mod modal-content">
            ...
            <table width="100%"  border="0" class="formlist" cellpadding="0" cellspacing="0">
             <tbody>
            	<tr>
                   <th width="30%" >
                    ...
                    <td >
                         <input id="voteId" name="voteId" type="hidden" value="${voteMainEntity.voteId}" />
                         <input id="voteTitle" maxlength="50" placeholder="不要超过50个字符哦(^_^)" name="voteTitle" type="text" value="${voteMainEntity.voteTitle }" class="form-control" readonly="readonly"/>
                   </td>
              </tr>
              <tr>
                  <th width="30%" >
                   ...
                   <td >
                        <textarea maxlength="200" placeholder="不要超过200个字符哦(^_^)" rows="4" cols="30" id="voteDescription" name="voteDescription" class="form-control">${voteMainEntity.voteDescription }</textarea>
                  </td>
              </tr>
              <tr id="voteTypeTr">
                 <th width="30%" >
                  ...
                  <td >
                     <select id="voteType" name="voteType"  class="form-control">
                         <option value="1" <c:if test="${voteMainEntity.voteType == '1' }"> selected="selected"</c:if>>打分</option>
	                     <option value="0" <c:if test="${voteMainEntity.voteType == '0' }"> selected="selected"</c:if>>文字</option>
              ...
              <c:choose> 
				  <c:when test="${voteMainEntity.voteType == '0' }"><!--如果 --> 
		                 ...
		                 <th width="30%" >
		                  ...
		                  <td >
		                     <select id="voteIsMultiSelect" name="voteIsMultiSelect"  class="form-control">
		                        <option value="1" <c:if test="${voteMainEntity.voteIsMultiSelect == '1' }"> selected="selected"</c:if>>是</option>
			                     <option value="0" <c:if test="${voteMainEntity.voteIsMultiSelect == '0' }"> selected="selected"</c:if>>否</option>
		                 ...
		                 <th width="30%" >
		                  ...
		                  <td >
		                     <select id="voteMaxChoice" name="voteMaxChoice"  class="form-control">
		                        <option value="2" <c:if test="${voteMainEntity.voteMaxChoice == '2' }"> selected="selected"</c:if>>2</option>
			                     <option value="3" <c:if test="${voteMainEntity.voteMaxChoice == '3' }"> selected="selected"</c:if>>3</option>
		             		</select>
		                 </td>
		              </tr>
				  </c:when>      
				  <c:otherwise>  <!--否则 -->    
					   <tr id="voteMaxScoreTr">
					        <th width="30%">
					                               最大分值
					        </th>
		                    <td ><select id="voteMaxScore" name="voteMaxScore"  class="form-control">
				        ...
				        <tr id="voteScoreMethod">
					        <th width="30%">
					                               分数统计方式
					        </th>
		                    <td ><select id="voteScoreMethod" name="voteScoreMethod"  class="form-control">
		                        <option value="1" <c:if test="${voteMainEntity.voteScoreMethod == '1' }"> selected="selected"</c:if>>总分</option>
			                     <option value="0" <c:if test="${voteMainEntity.voteScoreMethod == '0' }"> selected="selected"</c:if>>平均分</option>
		                    </td>
	                    </tr>
				  </c:otherwise> 
			  </c:choose>
             ...
             <tr>
                 <th width="30%" >
                  ...
                  <td >
                     <select name="voteIsAnonymous"  class="form-control">
                         <option value="1" <c:if test="${voteMainEntity.voteIsAnonymous == '1' }"> selected="selected"</c:if>>是</option>
	                     <option value="0" <c:if test="${voteMainEntity.voteIsAnonymous == '0' }"> selected="selected"</c:if>>否</option>
             		</select>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td >
                     <select name="voteResultOnlySee"  class="form-control">
                        <option value="1" <c:if test="${voteMainEntity.voteResultOnlySee == '1' }"> selected="selected"</c:if>>是</option>
	                     <option value="0" <c:if test="${voteMainEntity.voteResultOnlySee == '0' }"> selected="selected"</c:if>>否</option>
             		</select>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td >
                    <input type="text" id="voteEndDate" name="voteEndDate" class="form-control"  placeholder="请选择日期" value="<fmt:formatDate value="${voteMainEntity.voteEndDate }" pattern="yyyy-MM-dd HH:mm:ss"/>" readonly="readonly"/>  
                 </td>
              </tr>
               <tr>
                 <th width="30%" >
                  ...
                  <td >
                     <select name="voteIsDraft"  class="form-control">
                        <option value="1" <c:if test="${voteMainEntity.voteIsDraft == '1' }"> selected="selected"</c:if>>是</option>
	                     <option value="0" <c:if test="${voteMainEntity.voteIsDraft == '0' }"> selected="selected"</c:if>>否</option>
             		</select>
                 </td>
              </tr>
              <tr>
                 <th width="30%" >
                  ...
                  <td >
                     <select name="qzId"  class="form-control">
                         <c:forEach items="${qzEntities }" var="item">
                          <c:choose> 
				               <c:when test="${voteGroup.qzId == item.qzId }">
				                 <option value="${item.qzId}" selected="selected">${item.qzName }</option>
				               </c:when>      
				               <c:otherwise>
		                         <option value="${item.qzId}">${item.qzName }</option>
				               </c:otherwise>     
				          </c:choose>     
                         </c:forEach>
             		</select>
              ...
              <!-- 选项 -->
               <c:forEach items="${voteOptionEntities}" var="item">
                    <tr>
                        <th width="30%">
                                                                               选项
                        </th>
                        <td>
                            <input style="width:250px;height:30px;" maxlength="50" placeholder="不要超过50个字符哦(^_^)" name="option" type="text" value="${item.voteOptionDescription }"/>&nbsp;<span name="deleteOption" onclick="deleteSelfTr(this);" style="color:red;cursor:pointer;font-size:20px;">x</span>
                        </td>
                     </tr>
               </c:forEach>
