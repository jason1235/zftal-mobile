...
<html>
	...
	<body>
	    ...
	    <form id="form_list" role="form" action="<%=request.getContextPath() %>/zxgl/sceneryCatalog_list.html" method="post">
	        ...
	        <div class="row">
		        <div class="col-xs-3">
		            <div class="form-group">
		                ...
		                <div class="col-xs-7">
	                        <s:textfield name="query.sceneryCatalogName"  cssClass="form-control" theme="simple" maxlength="10" placeholder="最多10个字符"></s:textfield>
	                    </div>
		            </div>
		        </div>
		        <div class="col-xs-3">
		...
		<input name="maxIndex" id="maxIndex" type="hidden" value=""/>
		            	<font color="red">(^_^)提示：</font>	拖动条目并保存顺序，可变更移动学校风景栏目的显示次序
        <table class="table table-striped sl_all_bg table-bordered text-left">
		    ...
		    <s:iterator value="list" var="p" status="st">
                <tr class="pxtr">
                    ...
                    <input name="sceneryCatalogId" type="checkbox" id="id" value="${p.sceneryCatalogId }"/>
                    <input name="status" type="hidden" value="${p.isActive }" />
                     <input name="ids" type="hidden" value="${p.sceneryCatalogId }"/>
	                 <input name="sortNumber" type="hidden" value="${p.sortNumber}"/>   
                    </td>
                    <td>${p.sceneryCatalogName}</td>
                    <td>
		...
		var heightOffset= 200;
		    ...
		    $("#btn_zj").click(function(){
		        var url = "<%=request.getContextPath()%>/zxgl/sceneryCatalog_edit.html";
		        var data = {};
			 ...
			 $("#btn_xg").click(function(){
                    ...
                    var id = checked.val();
                    var url = "<%=request.getContextPath()%>/zxgl/sceneryCatalog_edit.html?query.sceneryCatalogId=" + id;
                    $("#myModal_toji").find(".modal-content").width(750);
                    $("#myModal_toji").find(".modal-content").height(200);
                    $("#myFrame").attr("src", url);
                    $("#myFrame").height(200);
				...
				function doUpdateIndex() {
					...
					$('#form_list').attr('action','<%=request.getContextPath()%>/zxgl/sceneryCatalog_updateIndex.html');
					$('#form_list').submit();
		     ...
		     $("#btn_sc").click(function(){
					...
					$("#why_sure").click(function(){
						$.post("<%=request.getContextPath() %>/zxgl/sceneryCatalog_delete.html?query.sceneryCatalogId="+id , null, function(data){
				...
				$('#btn_qy').click(function(){
					...
					var id = checked.val();
					$.post("<%=request.getContextPath() %>/zxgl/sceneryCatalog_updateActive.html?query.sceneryCatalogId="+id+"&query.isActive=1", null, function(data){
							var callback = function(){
								location.reload();
							};
							processCall(data, callback);
				...
				var checked=$("input[id='id']:checked");
					...
					var id = checked.val();
					$.post("<%=request.getContextPath() %>/zxgl/sceneryCatalog_updateActive.html?query.sceneryCatalogId="+id+"&query.isActive=0", null, function(data){
							var callback = function(){
								location.reload();
							};
							processCall(data, callback);
