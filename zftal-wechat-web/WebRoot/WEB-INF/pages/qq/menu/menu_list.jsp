...
<html>
  <head>
    ...
    <script type="text/javascript">
        ...
        function view2menuObj(){
            ...
            for(var i=0;i<menuList.length;i++){
                ...
                for(var j=0;j<subMenuList.length;j++){
                    ...
                    buttonList[i].sub_button[j]=menuInfo.button[no_menu].sub_button[no_sub];
                }
            }
            menuInfo.button=buttonList;
        }
        ...
        function viewMenu(){
        	...
        	$("#notFindMenu").show();
        	$("#menu_add").show();
        	if(menuInfo.button.length>0){
                $("#notFindMenu").hide();
                var buttonList=menuInfo.button;
                var html = "";
                var i=0;
        ...
        function addMenu(num){
            ...
            $("#saveMenu_btn").click(function(){
                ...
                if($("#menu_url_login")[0].checked){
                    url="https://open.mp.qq.com/connect/oauth2/authorize?appid=${appid}&redirect_uri="
                        +urlencode(url)
                        +"&response_type=code&scope=snsapi_base&state=yd_qq%23qq_redirect";
                }
                menu.url=url;
                var buttonList;
                if(num==null){
                	if(menuInfo.button==null){
                		menuInfo.button=new Array();
                    }
                    buttonList=menuInfo.button;
                }else{
                    if(menuInfo.button[num].sub_button==null){
                    	menuInfo.button[num].sub_button=new Array();
                    }
                	buttonList=menuInfo.button[num].sub_button;
                }
        ...
        function showMenuInfoWin(){
        	var html="<div class='tab'>"
                ...
                +"<tr id='menu_url_tr'><th>菜单链接</th><td><input id='menu_url' type='text' /><br><input id='menu_url_login' type='checkbox' />自动登录</td></tr>"
                +"</tr></tbody></table></div>";
        ...
        function saveMenu() {  
        	$("#menuInfo_val").val(JSON.stringify(menuInfo));
        	$.post('<%=request.getContextPath()%>/qq/menu_save.html',
                	$("#menuInfo_val").serialize(),function(data){
        		var callback = function(){
        			location.href = "<%=request.getContextPath()%>/qq/menu_list.html#";
                };
                if(data.success){
  ...
  <body>
    ...
    <div id="menu_save" class="demo_add">
        <a href="#" onclick="saveMenu()">保存并同步至QQ公众号</a>
    </div>
  </body>
</html>
