...
import com.zfsoft.wechat.common.entity.user.SNSUserInfo;
import com.zfsoft.wechat.common.entity.user.WeChatUserInfo;
...
public class WeChatUserUtil {
	...
	/**
	 * 通过网页授权获取用户信息
	 * 描述：拉取用户信息(需scope为 snsapi_userinfo)
	...
	public static SNSUserInfo getSNSUserInfo(String accessToken, String openId, String lang) {
		log.info("getSNSUserInfo START");
		SNSUserInfo snsUserInfo = null;
		// 拼接请求地址
		String requestUrl = WeChatConstants.WEB_USERINFO_URL;
		...
		if (null != jsonObject) {
			try {
				...
				log.error("getSNSUserInfo FAIL errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		log.info("getSNSUserInfo END");
		return snsUserInfo;
	}
	 ...
	 */
	public static WeChatUserInfo getUserInfo(String accessToken, String openId, String lang) {
		log.info("getUserInfo START");
		WeChatUserInfo weChatUserInfo = null;
		// 拼接请求地址
		String requestUrl = WeChatConstants.USERINFO_URL;
		requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken);
		requestUrl = requestUrl.replace("OPENID", openId);
		requestUrl = requestUrl.replace("LANG", lang);
		// 获取用户信息
		...
		if (null != jsonObject) {
			try {
				...
				if (0 == weChatUserInfo.getSubscribe()) {
					log.error("用户已取消关注", weChatUserInfo.getOpenId());
				} else {
					int errorCode = jsonObject.getInt("errcode");
					String errorMsg = jsonObject.getString("errmsg");
					log.error("getUserInfo FAIL errcode:{} errmsg:{}", errorCode, errorMsg);
				}
			}
		}
...
//	public static String WeixinOauthRedirectURL(String serviceURL , String scope ){
//		StringBuffer buffer  = new StringBuffer();
//		buffer.append("https://api.weixin.qq.com/sns/oauth2/authorize?");
//		buffer.append("appid=").append(CommonUtil.APPID).append("&");
//		buffer.append("redirect_url=").append(CommonUtil.urlEncodeUTF8(serviceURL)).append("&");
//		buffer.append("response_type=code").append("&");
