package com.zfsoft.mobile.interfaceManager.dao.query;

import java.util.Date;

import com.zfsoft.dao.query.BaseQuery;

public class InterfaceManagerQuery extends BaseQuery{

	/**
	 * 
	 */
	private static final long serialVersionUID = -4630141505543027618L;

	private String classJkid;
	
	private String classJkms;
	
	private String classJklm;
	
	private String classSyzt;
	
	private String classJkdz;
	
	private Date classCjsj;
	
	private Date classGxsj;
	
	private String classCjzid;
	
	private String classGxzid;
	
	private String classDeleted;
	
	private String classJkbm;

	public String getClassJkid() {
		return classJkid;
	}

	public void setClassJkid(String classJkid) {
		this.classJkid = classJkid;
	}

	public String getClassJkms() {
		return classJkms;
	}

	public void setClassJkms(String classJkms) {
		this.classJkms = classJkms;
	}

	public String getClassJklm() {
		return classJklm;
	}

	public void setClassJklm(String classJklm) {
		this.classJklm = classJklm;
	}

	public String getClassSyzt() {
		return classSyzt;
	}

	public void setClassSyzt(String classSyzt) {
		this.classSyzt = classSyzt;
	}

	public String getClassJkdz() {
		return classJkdz;
	}

	public void setClassJkdz(String classJkdz) {
		this.classJkdz = classJkdz;
	}

	public Date getClassCjsj() {
		return classCjsj;
	}

	public void setClassCjsj(Date classCjsj) {
		this.classCjsj = classCjsj;
	}

	public Date getClassGxsj() {
		return classGxsj;
	}

	public void setClassGxsj(Date classGxsj) {
		this.classGxsj = classGxsj;
	}

	public String getClassCjzid() {
		return classCjzid;
	}

	public void setClassCjzid(String classCjzid) {
		this.classCjzid = classCjzid;
	}

	public String getClassGxzid() {
		return classGxzid;
	}

	public void setClassGxzid(String classGxzid) {
		this.classGxzid = classGxzid;
	}

	public String getClassDeleted() {
		return classDeleted;
	}

	public void setClassDeleted(String classDeleted) {
		this.classDeleted = classDeleted;
	}

	public void setClassJkbm(String classJkbm) {
		this.classJkbm = classJkbm;
	}

	public String getClassJkbm() {
		return classJkbm;
	}
	
}
