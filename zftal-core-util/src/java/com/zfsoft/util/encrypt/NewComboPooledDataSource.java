 ...
 * </p>
 */
public final class NewComboPooledDataSource extends
		AbstractPoolBackedDataSource implements PooledDataSource, Serializable,
		Referenceable {
	final static MLogger logger = MLog
			.getLogger(NewComboPooledDataSource.class);
	...
	public NewComboPooledDataSource() {
		this(true);
	}

	public NewComboPooledDataSource(boolean autoregister) {
		super(autoregister);
		...
		try {
			...
			logger
					.log(
							MLevel.WARNING,
							"Hunh??? This can't happen. We haven't set up any listeners to veto the property change yet!",
							e);
	...
	private void updateLocalVarsFromCpdsProp() {
		this.wcpds = (WrapperConnectionPoolDataSource) this
				.getConnectionPoolDataSource();
		this.dmds = (DriverManagerDataSource) wcpds.getNestedDataSource();
	}

	public NewComboPooledDataSource(String configName) {
		this();
	...
	public void setJdbcUrl(String jdbcUrl) {
		...
		dmds.setJdbcUrl(dbencrypt.dCode(jdbcUrl.getBytes()));
	...
	public void setUser(String user) {
		...
		DBEncrypt dbencrypt = new DBEncrypt();
		dmds.setUser(dbencrypt.dCode(user.getBytes()));
	...
	public void setPassword(String password) {
		DBEncrypt dbencrypt = new DBEncrypt();
		// System.out.println(dbencrypt.dCode(password.getBytes()));
		dmds.setPassword(dbencrypt.dCode(password.getBytes()));
		this.resetPoolManager(false);
	}
	...
	public String toString() {
		...
		sb.append(" [ ");
		try {
			BeansUtils.appendPropNamesAndValues(sb, this,
					TO_STRING_IGNORE_PROPS);
		} catch (Exception e) {
